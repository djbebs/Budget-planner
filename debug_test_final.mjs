...
// Test with the correct parameters from the screenshots
const currentAmount = "5000";  // Starting balance (CHF 5000.00 from screenshots)
const targetAmount = "9000";   // Target balance (CHF 9000.00 from requirements)
const adjustmentCycleYears = "1";

console.log("=== FINAL TEST WITH CORRECT TARGET BALANCE ===");
...
try {
  ...
  // Check if final balance is close to target
  const finalBalance = lastMonth.endBalance;
  const targetBalance = parseFloat(targetAmount);
  const difference = Math.abs(finalBalance - targetBalance);
  const percentDifference = (difference / targetBalance) * 100;
  
  console.log("");
  console.log("=== BALANCE CONTROL ANALYSIS ===");
  console.log(`Target Balance: ${targetBalance.toFixed(2)}`);
  console.log(`Final Balance: ${finalBalance.toFixed(2)}`);
  console.log(`Difference: ${difference.toFixed(2)} (${percentDifference.toFixed(1)}%)`);
  
  if (percentDifference < 10) {
    console.log("✅ SUCCESS: Final balance is within 10% of target!");
  } else if (percentDifference < 25) {
    console.log("⚠️  WARNING: Final balance is within 25% of target (acceptable)");
  } else {
    console.log("❌ ISSUE: Final balance is more than 25% away from target");
  }

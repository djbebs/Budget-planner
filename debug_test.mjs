...
try {
  ...
  // Also check if we can find the exact pattern from the screenshots
  console.log("");
  console.log("=== LOOKING FOR SCREENSHOT PATTERN ===");
  const jun2025 = results.find(r => r.month.includes('Jun') && r.month.includes('2025'));
  if (jun2025) {
    console.log(`Jun 2025: Start=${jun2025.startBalance.toFixed(2)}, Savings=${jun2025.monthlySaving.toFixed(2)}`);
  }
} catch (error) {
